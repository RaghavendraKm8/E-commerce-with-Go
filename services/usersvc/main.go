...
type User struct { ID int64 `json:"id"`; Email string `json:"email"`; Age int `json:"age"` }
...
func main() {
    ...
    if err != nil { lg.Fatal("db", zap.Error(err)) }
    defer db.Close()
    ...
    go func(){ if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed { lg.Fatal("srv", zap.Error(err)) } }()
...
func get(c *gin.Context){
    id, _ := strconv.ParseInt(c.Param("id"),10,64); var u User
    if err := db.QueryRow(c,
